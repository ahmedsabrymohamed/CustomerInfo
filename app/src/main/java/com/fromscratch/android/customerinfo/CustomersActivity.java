      ...
      //  Log.d("ahmed", "onCreate: "+branch_address);
        ...
        mDatabaseReference2.addChildEventListener(new ChildEventListener() {
            ...
            public void onChildAdded(DataSnapshot dataSnapshot, String s) {
                branches_map.put(dataSnapshot.getKey().toString(),dataSnapshot.getValue(Branch.class).address);

            }

            @Override
            public void onChildChanged(DataSnapshot dataSnapshot, String s) {
                branches_map.remove(dataSnapshot.getKey().toString());
                branches_map.put(dataSnapshot.getKey().toString(),dataSnapshot.getValue(Branch.class).address);
            }

            @Override
            public void onChildRemoved(DataSnapshot dataSnapshot) {
                branches_map.remove(dataSnapshot.getKey());
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        if (admin) {
            ...
            intent.putExtra("Coustomertxt_branch_address", branchcustomers.get(position).branch_address);
                     // Log.d("ahmed", "onItemClick: "+branchcustomers.get(position).branch_address);
            startActivity(intent);
